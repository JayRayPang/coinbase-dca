...
def dca():
    for coin in weights:
        ...
        for multiplier in multipliers:
            limit_price = (multiplier * price).quantize(round, ROUND_DOWN)
            limit_size = ((money * weights[coin]) / (limit_price * len(multipliers))).quantize(round, ROUND_DOWN)
            if (price + 1).log10().quantize(integer_round, ROUND_UP) >= 4:
            ...
            if 'message' in response and response['message'] == 'Insufficient funds':
                ...
                if balance >= Decimal('5.00'):
                    ...
                    limit_size = ((balance * weights[coin]) / limit_price).quantize(round, ROUND_DOWN)
                    if price.log10().quantize(integer_round, ROUND_UP) >= 4:
